import {inBounds,pavementHeight,DISTRICT_PROPS} from './district.js?v=20260911-s06a';
import {onWater,WATERFRONT_PROPS} from './waterfront.js?v=20260911-s06a';
...
export const PROPS=[...[[-8.9,-9],[-8.9,19],[-8.8,-32],[9,-26],[9,22],[-8.9,-50],[9,-48]].map(([x,z])=>({x,z,w:.32,d:.32,h:4.5})),...[-1,1].flatMap(side=>[-62,-46,-29,-10,18,28,38].map(z=>({x:side*7,z,w:.4,d:.4,h:14}))),...DISTRICT_PROPS,...WATERFRONT_PROPS,{x:6.8,z:7,w:.45,d:.35},{x:-6.8,z:-7,w:.45,d:.35},...[12,15,19].map(x=>({x,z:7.5,w:1.8,d:.9}))];
export function blocked(x,z,layout,r=.3){if(!inBounds(x,z)||onWater(x,z,r)||layout.dynamicBlocked?.(x,z,r))return true;return [...layout.buildings,...layout.cars,...PROPS,...(layout.obstacles?.()??[])].some(b=>Math.abs(x-b.x)<b.w/2+r&&Math.abs(z-b.z)<b.d/2+r);}
...
export function cameraDistance(p,desired,layout,height=floorAt(p.x,p.z)){const t={x:p.x,y:height+1.35,z:p.z};for(let i=1;i<=35;i++){const a=i/35,x=t.x+(desired.x-t.x)*a,z=t.z+(desired.z-t.z)*a,y=t.y+(desired.y-t.y)*a;const hit=layout.buildings.some(b=>Math.abs(x-b.x)<b.w/2+.15&&Math.abs(z-b.z)<b.d/2+.15&&y<b.h+.3)||PROPS.some(b=>b.w<.5&&Math.abs(x-b.x)<b.w/2+.18&&Math.abs(z-b.z)<b.d/2+.18&&y<(b.h??1));if(hit)return Math.max(.18,(i-2)/35);}return 1;}
