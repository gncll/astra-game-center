...
export function buildPark( scene, cells, { groundColor = 0x7ba45a, scatter = KENNEY_SCATTER, boundsOverride = null } = {} ) {
	...
	const bounds = boundsOverride || computeParkBounds( used, scatter );
	...
	// Floor edge trim omitted: the upstream full-area strip covered the slab.
	...
	await Promise.all( scatter.map( async ( spec ) => {
		...
		try {
			...
			try {
				...
				const gen = spec.useRecipe ? {colliders:[]} : await generateCollidersFor( spec.asset );
