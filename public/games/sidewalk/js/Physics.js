...
export function createContactTracker( skaterBody ) {
 let clock=0,last=-Infinity;
 function contact(a,b,manifold){
  if(a!==skaterBody&&b!==skaterBody)return;
  // Manifold normal points A → B; support must point upward toward the rider.
  const up=(a===skaterBody?-1:1)*manifold.worldSpaceNormal[1];
  if(up>.28)last=clock;
 }
 return { listener:{onContactAdded:contact,onContactPersisted:contact},
  advance(dt){clock+=dt*1000;}, reset(){last=-Infinity;},
  isGrounded(coyoteMs=80){return clock-last<coyoteMs;},
  debug(){return {clock,sinceContact:clock-last};}
 };
}
