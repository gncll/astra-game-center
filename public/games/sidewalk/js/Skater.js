...
const PUSH_ASSIST_RATE = 3.2;
const GROUND_PUSH_RESPONSE = 3.2;
const COAST_RESPONSE = 1.35;
const PUSH_TORQUE = 90;
...
export class Skater {
	...
	update( dt, input, groundCheck ) {
		...
		this.grindCooldown = Math.max(0,(this.grindCooldown || 0)-dt);
		...
		if ( input.touchActive && ( this.inputX !== 0 || this.inputZ !== 0 ) ) {
			...
			if ( targetSpeed < 0 && this.linearSpeed > 0.01 ) {
				...
				const rate = this.grounded
					? ( targetSpeed > 0 ? GROUND_PUSH_RESPONSE : COAST_RESPONSE )
					: 0.25;
		...
		if ( this.rigidBody ) {
			...
			const drive = this.linearSpeed * PUSH_TORQUE * dt * ( this.grounded ? 1 : 0.08 );
	...
	_respawn() {
		...
		if ( this.rigidBody ) {
			...
			rigidBody.setPosition( this.physicsWorld, this.rigidBody, this.spawnPosition || [ 0, 0.8, 0 ], false );
		...
		this.spherePos.set(...(this.spawnPosition || [0, .8, 0]));
		...
		this.comboScore = 0;
		this.comboLabel = '';
		this.comboTimer=0; this.ollieQueued=null; this.grounded=false; this.prevGrounded=false; this._airTricksThisHop.length=0;
	...
	_applyGrindLock( dt ) {
		...
		if ( Math.abs( along ) > halfLength + 0.25 ) {
		...
		const clampedAlong = along; // Preserve forward motion so the rider exits at the rail end.
		_grindTarget.set(
