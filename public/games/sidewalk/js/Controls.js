...
export class Controls {
	...
	constructor() {
		...
		this.keys = {}; this.pressed = new Set(); this.released = new Set();
		this.x = 0;
		this.z = 0;
		...
		window.addEventListener( 'keydown', ( e ) => {
			...
			if ( [ 'Space', 'KeyJ', 'KeyK', 'KeyL', 'ShiftLeft', 'ShiftRight', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight' ].includes( e.code ) ) e.preventDefault();

		} );
		window.addEventListener( 'keyup', ( e ) => { this.keys[ e.code ] = false; this.released.add(e.code); } );

		window.addEventListener('blur', () => this.clear());
		if (matchMedia('(pointer: coarse)').matches) this.setupTouchUI();

	}

	clear() { this.keys={}; this.pressed.clear(); this.released.clear(); this.touchOllie=this.touchFlip=this.touchGrab=this.touchGrind=false; this.touchActive=false; this._prev={space:false,flip:false,grab:false,grind:false,stance:false}; }
	...
	update() {
		...
		const events = {
			...
			ollieUp:   (! spaceHeld && this._prev.space) || this.released.has('Space'),
			flipDown:  (flipHeld && ! this._prev.flip) || this.pressed.has('KeyJ'),
		...
		this.touchFlip = false; this.pressed.clear(); this.released.clear();
